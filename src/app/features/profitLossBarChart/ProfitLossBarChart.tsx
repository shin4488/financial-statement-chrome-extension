...
export default class ProfitLossBarChart extends React.Component<ProfitLossBarChartProps> {
  ...
  hasInvalidData(): boolean {
    ...
    const debit = amount.originalCost + amount.sellingGeneralExpense + amount.operatingIncome;
    return (
      ...
      >
        ...
        <Bar dataKey="originalCostAmount" stackId="a" fill="#FEBBCC">
          <LabelList
            ...
            formatter={(value: number) =>
              `${dataKeyJapaneseHash.originalCostAmount}: ${value.toLocaleString()}%`
            }
          />
        </Bar>
        <Bar dataKey="sellingGeneralExpenseAmount" stackId="a" fill="#E48586">
          <LabelList
            ...
            position="center"
            formatter={(value: number) =>
              `${dataKeyJapaneseHash.sellingGeneralExpenseAmount}: ${value.toLocaleString()}%`
            }
          />
        </Bar>
        ...
        <Bar dataKey="netSalesAmount" stackId="a" fill="#A1C2F1">
          <LabelList
            ...
            position="center"
            formatter={(value: number) =>
              `${dataKeyJapaneseHash.netSalesAmount}: ${value.toLocaleString()}%`
            }
          />
        </Bar>
        ...
        {hasLoss ? (
          <Bar dataKey="operatingLossAmount" stackId="a" fill="#F7C04A">
            <LabelList
              ...
              formatter={(value: number) =>
                `${dataKeyJapaneseHash.operatingLossAmount}: -${value.toLocaleString()}%`
              }
              position="center"
              fill={stackLabelListFillColor}
            />
          ...
          <Bar dataKey="operatingIncomeAmount" stackId="a" fill="#8EC3B0">
            <LabelList
              ...
              formatter={(value: number) =>
                `${dataKeyJapaneseHash.operatingIncomeAmount}: ${value.toLocaleString()}%`
              }
              position="center"
              fill={stackLabelListFillColor}
            />
