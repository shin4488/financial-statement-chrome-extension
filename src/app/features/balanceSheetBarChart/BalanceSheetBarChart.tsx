...
import { stackLabelListFillColor } from '@/app/constants/values';
import { BalanceSheetBarChartProps } from './props';
import { BalanceSheetAmountKeyLabel, BalanceSheetChart } from './chartData';
import FinancialStatementBarChart from '@/app/features/financialStatementBarChart/FinancialStatementBarChart';
...
export default class BalanceSheetBarCahrt extends React.Component<BalanceSheetBarChartProps> {
  ...
  hasInvalidData(): boolean {
    ...
    const credit = amount.currentLiability + amount.noncurrentLiability + amount.netAsset;
  ...
  render(): React.ReactNode {
    ...
    if (isInsolvency) {
      balanceSheetCharData.push({
        ...
        blanckForInsolvencyAmount: amount.currentLiability + amount.noncurrentLiability + netAsset,
    ...
    return (
      <FinancialStatementBarChart
        ...
        tooltipFormatter={(value, name) => {
          const balanceSheetAmountKey = name as keyof BalanceSheetAmountKeyLabel;
          return [
      ...
      >
        ...
        <Bar dataKey="currentAssetAmount" stackId="a" fill="#A1C2F1">
          <LabelList
            ...
            position="center"
            formatter={(value: number) => `${dataKeyJapaneseHash.currentAssetAmount}: ${value}%`}
          />
        </Bar>
        <Bar dataKey="propertyPlantAndEquipmentAmount" stackId="a" fill="#5A96E3">
          <LabelList
        ...
        <Bar dataKey="intangibleAssetAmount" stackId="a" fill="#7286D3">
          <LabelList
            ...
            fill={stackLabelListFillColor}
            position="center"
            formatter={(value: number) => `${dataKeyJapaneseHash.intangibleAssetAmount}: ${value}%`}
          />
        </Bar>
        ...
        {isInsolvency ? (
          ...
          <Bar dataKey="blanckForInsolvencyAmount" stackId="a" fill="transparent" />
        ) : (
          <></>
        )}
        <Bar dataKey="netAssetAmount" stackId="a" fill="#8EC3B0">
          <LabelList
            ...
            position="center"
            formatter={(value: number) =>
              `${dataKeyJapaneseHash.netAssetAmount}: ${isInsolvency ? -Number(value) : value}%`
            }
          />
        </Bar>
      </FinancialStatementBarChart>
